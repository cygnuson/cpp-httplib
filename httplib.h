...
public:
  ...
  using OrphanedSocketHandler = std::function<void(int)>;
  ...
  inline void
  AutoOrphanSocket(const OrphanedSocketHandler &handle_orphaned_socket);

  inline void AssumeOrphanedSocket(int sock);
...
private:
  ...
  OrphanedSocketHandler handle_orphaned_socket_;
...
inline bool Server::listen_internal() {
...

  {
    ...
    while (svr_sock_ != INVALID_SOCKET) {
      ...
      if (handle_orphaned_socket_) {
        (handle_orphaned_socket_)(sock);
      } else {
...
inline void Server::AssumeOrphanedSocket(int sock) {
  std::unique_ptr<TaskQueue> task_queue(new_task_queue());
...
inline void
Server::AutoOrphanSocket(const OrphanedSocketHandler &handle_orphaned_socket) {
  handle_orphaned_socket_ = handle_orphaned_socket;
}
